  ...
  const fetchWithoutCache = useCallback(
    async <TData, TParams extends object = object>(
      ...
      wrappedRequest<TData>(async () => {
        ...
        //       const cacheValues = JSON.parse(cache?.current.get(key) || '');
        //       cacheValues.data.forEach((curr: any) => {
        //         // @ts-ignore
        ...
        //         }
        //       });
        //       // cacheValues.data = updatedValues;
        //       const finalString = JSON.stringify(cacheValues);

        //       // cache.current.set(key, finalString);
        //     }
        //   }
        // }
        clearCache();
